        ...
        try:
            ...
            if media_group_id:
                ...
                if result:
                    ...
                    return
            # Check for existing pending photo with this message_id
            self.cursor.execute(
                "SELECT id FROM pending_photos WHERE user_id = %s AND message_id = %s",
                (user_id, message_id)
            )
            if self.cursor.fetchone():
                self.cursor.execute(
                    "UPDATE pending_photos SET photo_ids = %s, media_group_id = %s WHERE user_id = %s AND message_id = %s",
                    (photo_ids_str, media_group_id, user_id, message_id)
                )
            else:
                self.cursor.execute(
            ...
            self.conn.commit()
            print(f"Debug - Successfully logged pending photo: user_id={user_id}, message_id={message_id}, photos={photo_ids_str}")
        except mysql.connector.Error as e:
    ...
    def get_pending_photos(self, user_id, media_group_id=None):
        try:
            if media_group_id:
                ...
                self.cursor.execute(
                    "SELECT message_id, photo_ids, media_group_id FROM pending_photos WHERE user_id = %s ORDER BY message_id ASC",
                    (user_id,)
                )
            return self.cursor.fetchall()
        except mysql.connector.Error as e:
